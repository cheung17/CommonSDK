...
public class ListUtils {
     ...
     * @return 是否相等
     */
    public static <T extends Comparable<T>> boolean compareWithSequence(List<T> a, List<T> b) {
        if (a.size() != b.size())
            return false;
        for (int i = 0; i < a.size(); i++) {
            if (!a.get(i).equals(b.get(i)))
     ...
     * @return 是否相等
     */
    public static  boolean compareWithSequenceIgnoreCase(List<String> a, List<String> b) {
        if (a.size() != b.size())
            return false;
        for (int i = 0; i < a.size(); i++) {
